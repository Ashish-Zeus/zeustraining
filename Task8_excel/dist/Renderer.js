...
export class Renderer {
     ...
     * @param data
     */
    constructor(ctx, cfg, data, colW, rowH) {
        this.ctx = ctx;
        this.cfg = cfg;
        this.data = data;
        this.colW = colW;
    ...
    drawBody(vp, offX, offY, sel) {
        ...
        if (sel instanceof RangeSelection && !sel.isSingle()) {
            ...
            ctx.fillRect(sel.anchorCol * cfg.defaultColWidth, // ← fixed column
            0, // first row
            cfg.defaultColWidth, cfg.defaultRowHeight);
        }
    ...
    drawRowHeaders(vp, _oy, sel) {
        ...
        for (let r = firstRow; r < cfg.rows && y < vp.height; r++) {
            const h = this.rowH[r];
            /* classification */
            const inCellSel = (sel instanceof CellSelection && sel.row === r) ||
                (sel instanceof RangeSelection && r >= sel.r0 && r <= sel.r1);
            const inHdrSel = (sel instanceof RowSelection && sel.row === r) ||
                (sel instanceof RowRangeSelection && r >= sel.r0 && r <= sel.r1);
            const nextHdrSel = (sel instanceof RowSelection && sel.row === r + 1) ||
                (sel instanceof RowRangeSelection &&
                    r + 1 >= sel.r0 &&
                    r + 1 <= sel.r1);
            const prevHdrSel = (sel instanceof RowSelection && sel.row === r - 1) ||
                (sel instanceof RowRangeSelection &&
                    r - 1 >= sel.r0 &&
                    r - 1 <= sel.r1);
            /* fills */
